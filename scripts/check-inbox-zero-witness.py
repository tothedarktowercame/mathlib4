#!/usr/bin/env python3
"""Check the model and five deliberately false mutant claims; no runtime calls."""
import hashlib
import json
...
MODEL = 'DarkTower/WarMachine/InboxZeroWitness.lean'
BASE = ROOT / 'DarkTower/WarMachine/inbox-zero-witness'


def check():
    rows = []
    commands = [('build', ['lake', 'build', 'DarkTower.WarMachine.InboxZeroWitness'], 0),
                ('model', ['lake', 'env', 'lean', MODEL], 0)]
    # Fixed population: a missing negative must not silently reduce the gate.
    for name in ['T1Silent', 'T2StaleFeel', 'T3SilentProgress', 'T3NewDirt', 'T4ZeroCommit']:
