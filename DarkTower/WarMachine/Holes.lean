...
/-- CLOSED-BY-STRUCTURE 2026-09-20 · was: DEFERRAL UNDER ORGANIZED DISCOVERY (HOLE) · owner: RULINGS-walkthrough-2026-09-08.md Item 17, approved RULINGS-walkthrough-2026-09-09.md Item 18c · closure authorized: Joe, 2026-09-20 ("yes, close C591 as proposed"), futon2 holes/labs/wm-contract/PROPOSAL-c591-closure-2026-09-20.md · evidence: futon2 holes/labs/wm-contract/C591-C-as-calculation-proposal.md; excursion futon2 holes/E-C-realization.md · The deferral text fixed the structure — "a time-indexed Cτ family whose terminal member is the ruled outcome-kind distribution" — and that structure now runs in production (step-indexed family, terminal member the declared PreferenceSpec, certified `:varies-across-horizon`; conformance SPEC-c rows 1–9, declared `:c-schedule`). This definition states exactly that structure and NOTHING about values: the terminal member and the elsewhere family are PARAMETERS, honoring "does not license choosing its value" — which preference applies remains a per-declaration runtime fact with provenance. It is the abstract-observation form of `ZeroPreferenceExclusion.preferenceAt`, whose token-space instance carries the proofs (`preferenceAt_nonneg`, `preferenceAt_sum`). Preferences remain declared per PRAGMATIC vertex only (C122's census). STILL OPEN, deliberately: the disposition bridge P(d|o), to be fitted from recorded observation-to-disposition evidence — that sub-obligation is untouched by this closure and retains its by-record holder. -/
def C {Obs : Vertex → Type*} (v : Vertex) (_pragmatic : v ≠ Vertex.evidence)
    (terminal : Obs v → ℝ) (τ : ℕ) (D : ℕ → Obs v → ℝ) : ℕ → Obs v → ℝ :=
  fun n => if n = τ then terminal else D n
