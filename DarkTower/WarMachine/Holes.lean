...
/-- PERMANENT EXTERNAL ATTESTATION · Lean cannot prove an event · evidence is the executable witness · contract kind HOLE intentionally · owner: wm-organization · TN-edge-review worklist H3, from Joe's J1 ruling · holder: by-record · evidence: a run record carrying τ together with the β it was derived from · falsifier: a run record in which τ is set by the β update from G and π · THE MACHINE'S DIAL IS NOT THE FORMALISM'S. Friston 2017 eq. 2.7 and Da Costa 2020 A.2 give policy precision as γ = 1/β with β ← β + (π − π₀)·G — the precision learns from the policy posterior's departure from its prior, weighted by expected free energy. What the machine uses instead is τ from the score spread and an engineering selection gain (`futon2/src/futon2/aif/policy.clj:242-245`), and `selection_gain.clj` says in its own words that this "is not Friston's variational policy precision". Live, the gain does not even move: the fold returns the state unchanged unless a realised outcome is well-formed and new, and the field "is ABSENT today, sim-only", so τ holds at its prior (`futon2/src/futon2/aif/selection_gain.clj:187-193`). No β appears anywhere in the policy path. J1 ruled the drawn R7→R14 was a conflation and that the theory-aligned precision is to be pursued; this is the claim held open: policy precision is γ = 1/β updated by eq. 2.7. -/
def policyPrecisionIsGammaFromBeta : Prop := sorry

/-- PERMANENT EXTERNAL ATTESTATION · Lean cannot prove an event · evidence is the executable witness · contract kind HOLE intentionally · owner: wm-organization · TN-edge-review worklist H4, from Joe's J2 ruling · holder: by-record · evidence: a run record whose Q(π) carries a per-policy F term alongside E and G · falsifier: a run record in which Q(π) is computed with an F_π term from the observations under each policy · TWO DIFFERENT FREE ENERGIES, AND THE MACHINE COMPUTES NEITHER WHERE THE FORMALISM NEEDS IT. Parr 2022 B.9 gives the policy posterior as π = σ(ln E − F − G), where F_π is the variational free energy of the observations under each policy. The machine's policy score is −G/τ + log-prior (`futon2/src/futon2/aif/policy.clj:242-246`) — there is no F term in it, and no free-energy symbol anywhere in the scoring path. The Laplace channel F that R8 does compute is a different quantity and a diagnostic: it is bound at `futon2/scripts/futon2/report/war_machine.clj:4450-4451`, route-tagged :R8 at :4452, and appears exactly once more, in the report map at :4753 — downstream of scoring, consumed by nothing (C448, C452, D1). So the free energy that exists is not consumed, and the free energy the posterior needs is not computed. J2 ruled on the composition; this is the claim held open: the policy posterior imports F_π. -/
