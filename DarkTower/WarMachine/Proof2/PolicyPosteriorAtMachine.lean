...
/-! ## The weights at the counted habit prior (W10, R17→R6)

`machineWeights` takes `habit : PolicyIndex → ℝ` as a free binder. C4's
`EnactmentHabit.habitPrior` (mathlib4 `8f5e11fe78`) is the counted E: `(count + α)`
normalised over the distinct menu entries, uniform with no data (through `α = 1`), and a
zero-count policy has mass `α / Z > 0`. Nothing applied the one to the other, so the
registry edge R17→R6 (E into the posterior) had the term in the consumer's signature and
no import behind it. This section applies it, in this module because the row's `:lean`
(`machinePosterior`) lives here and the edge imports only if the module that holds the
row's term imports E's.

The menu is the policy list, `PolicyIndex := PolicyKey M P`. `habitPrior` needs a
non-empty menu and the concentration `α` (`Concentration`); both are arguments (the
requisition's signature omitted `α`, which `habitPrior` cannot be applied without). An
EMPTY menu is `habitPrior`'s hypothesis failing, so it is an absence
(`HabitAbsence.emptyMenu`), not a default weight list. -/
...
/-- Every menu policy has positive habit mass at the counted prior, so `machineWeights`'
carrier hypothesis `0 < habit π` is met on a non-empty menu. -/
...
theorem unenactedPolicyIsScored (records : List (Record M P)) (c : Concentration)
    ...
    (∃ ws, machineWeightsAtHabit records c grade F tau policies = .ok ws ∧
      weight (habitAt records c policies hne) grade F tau k
          / normaliser (habitAt records c policies hne) grade F tau policies ∈ ws) := by
