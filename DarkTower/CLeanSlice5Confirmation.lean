...
namespace CLeanProof_slice5_confirmation
-- slice5-confirmation: Do any AIF terms in the ant forager carry a measurable explore/exploit effect?
...
def spine : BV Method :=
  BV.seq (BV.atom Method.freezeEnvironment) (BV.seq (BV.atom Method.fixInterpretation) (BV.seq (BV.atom Method.verifyInstrument) (BV.seq (BV.atom Method.measureTreatments) (BV.atom Method.adjudicate))))
...
example : BV.Cong
    (BV.seq (BV.seq (BV.atom Method.freezeEnvironment) (BV.atom Method.fixInterpretation)) (BV.atom Method.verifyInstrument))
    (BV.seq (BV.atom Method.freezeEnvironment) (BV.seq (BV.atom Method.fixInterpretation) (BV.atom Method.verifyInstrument))) :=
  BV.Cong.seq_assoc (BV.atom Method.freezeEnvironment) (BV.atom Method.fixInterpretation) (BV.atom Method.verifyInstrument)
...
end CLeanProof_slice5_confirmation

namespace CLeanExperiment_slice5_confirmation
-- slice5-confirmation: Do any AIF terms in the ant forager carry a measurable explore/exploit effect?
...
/-- computed from (:var mu), identical across candidate actions (policy.clj:612-624); a common addend cannot change an argmax. Proved in DarkTower/Slice5Preregistration.lean as ambiguity_ablation_preserves_selection. -/
...
/-- food_belief.clj:101-149; action-dependent, so it can reorder candidates. -/
...
/-- action-dependent; pilot showed ablating it nearly triples sparse starvation (0.233 -> 0.667) while raising yield. -/
...
inductive Outcome where
  ...
  | noTermCarriesEffect
  | exploreExploitRegulatorEstablished
  | riskCarriesEffectEpistemicDecorative
  | reportPerScenarioNoGlobalClaim
  | pilotOnlyNotInterpretable
  deriving DecidableEq, Repr
...
def decisionRule : DecisionRule ExperimentTrace Outcome where
  name := "slice5-confirmation-decision"
  classify := fun trace => match trace.classification with
    | TraceClass.controlViolated => Outcome.abandonedAnalysisInvalid
    | TraceClass.noArmSignificant => Outcome.noTermCarriesEffect
    | TraceClass.epistemicSignificant => Outcome.exploreExploitRegulatorEstablished
    | TraceClass.riskOnlySignificant => Outcome.riskCarriesEffectEpistemicDecorative
